...
class _EditBoothWidgetState extends State<EditBoothWidget> {
  ...
  void getData() async {
    ...
    String respbody = (await http.get(
      Uri.parse(ServerInfo.addr + "/api/booth/"),)).body;
    List boothLocations = jsonDecode(respbody);
    Logger().v(boothLocations.toString());
    Logger().v(boothLocations.length);
    int? userBoothid = await credential.getBoothID();
    for (int i = 0; i < boothLocations.length; i++) {
      if (boothLocations[i]['id'] == userBoothid) {
        setState(() {

          textController1?.text = boothLocations[i]['name'];
          textController2?.text = boothLocations[i]['description'];
        });
        break;
      }
    }
    ...
    void setData() async {
      ...
      http.Response resp = await http.put(
        Uri.parse(ServerInfo.addr + "/api/booth/" + boothId.toString()),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({"name": textController1?.text.toString(),
      ...
      if (resp.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(resp.body)));
      }
